import { Effect, EffectComposer, EffectPass, RenderPass, Pass } from 'postprocessing';
...
const createLiquidEffect = (texture: THREE.Texture, opts?: { strength?: number; freq?: number }) => {
  const fragment = `
    ...
    `;
  const uniformMap = new Map<string, THREE.Uniform>([
  ...
  return new Effect('LiquidEffect', fragment, {
    uniforms: uniformMap as unknown as Map<string, THREE.Uniform<unknown>>
  });
};
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        const noiseUniformMap = new Map<string, THREE.Uniform>([
          ['uTime', new THREE.Uniform(0)],
        ...
        const noiseEffect = new Effect(
          ...
          {
            uniforms: noiseUniformMap as unknown as Map<string, THREE.Uniform<unknown>>
          }
        );
      ...
      const animate = () => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p: Pass) => {
            const passRecord = p as unknown as Record<string, unknown>;
            const effs = passRecord['effects'];
            if (Array.isArray(effs)) {
              (effs as Effect[]).forEach((eff: Effect) => {
